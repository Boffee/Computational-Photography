...
for c = 1:b1
    ...
    figure(5), imagesc(log(abs(fftshift(im1_fft)))), title('im1'), colormap jet;
    figure(6), imagesc(log(abs(fftshift(im2_fft)))), title('im2'), colormap jet;
    % filtered fft images
    im1_fil_fft = im1_fft .* fil_low_fft;
    im2_fil_fft = im2_fft .* (1 - fil_high_fft);

    figure(7), imagesc(log(abs(fftshift(im1_fil_fft)))), title('im1 low pass'), colormap jet;
    figure(8), imagesc(log(abs(fftshift(im2_fil_fft)))), title('im2 high pass'), colormap jet;
    % filtered images
